---
title: "Spatial Video, Immersive Video, VR180, 360Video, 3D-Foto oder volumetrisches Video – was ist der Unterschied?"
slug: was-ist-spatial-video-vr180-3d-foto-volumetrisches-video
status: published
primary_tag: visual-sales
tags: [visual-sales, glossar, spatial-computing, 3d-visualisierung]
meta_title: "Spatial Video, VR180 & 360° Video: Formate im Vergleich [2026]"
meta_description: "Spatial Video, VR180, 360°, 3D-Foto oder volumetrisches Video — was ist der Unterschied? Alle immersiven Videoformate für B2B und Content kurz erklärt, mit Vergleichstabelle."
feature_image: /assets/images/2025/11/Spatial_Video.jpeg
author: "Gerhard Schröder"
author_slug: gerhard
author_image: /assets/images/2025/10/1706254155883-1.jpeg
author_bio: "Gerhard Schröder ist Gründer & GF der viSales GmbH, Bochum. Seit über 15 Jahren spezialisiert auf visuelle Vertriebskommunikation für Maschinenbau & AeroSpace — mit 3D-Visualisierung & OpenUSD. Mitglied der AOUSD, Speaker zu AR & Spatial Sales."
published_at: "2025-04-25T06:00:00.000Z"
type: post
template: post
faq:
  - q: "Was ist Spatial Video?"
    a: "Spatial Video ist ein 3D-Videoformat, das zwei leicht versetzte Perspektiven aufzeichnet – ähnlich dem menschlichen Sehen. Es erzeugt beim Betrachten ein echtes Tiefengefühl. Apple hat Spatial Video für iPhone 15 Pro und Apple Vision Pro eingeführt."
  - q: "Was ist der Unterschied zwischen Spatial Video und VR180?"
    a: "Spatial Video zeigt ein fixes 3D-Bild ohne freie Blickbewegung – ideal für iPhone und Vision Pro. VR180 bietet ein breiteres Sichtfeld (180° horizontal) mit Stereoskopie für VR-Headsets wie Meta Quest. Beide nutzen zwei Linsen, unterscheiden sich in Blickfreiheit und Zielgerät."
  - q: "Welche Kamera brauche ich für Spatial Video oder VR180?"
    a: "Für Spatial Video genügt ein iPhone 15 Pro oder neuer. VR180 wird z.B. mit der Kandao QooCam 8K oder der Insta360 EVO aufgenommen. Für professionelle Produktionen gibt es Rig-Setups aus zwei synchronisierten Kameras."
  - q: "Auf welchen Geräten kann ich Spatial Video abspielen?"
    a: "Spatial Video läuft nativ auf dem Apple Vision Pro und auf dem iPhone 15 Pro im 3D-Modus. VR180 läuft auf Meta Quest, PlayStation VR und anderen VR-Headsets. Beide Formate lassen sich auch auf normalen Bildschirmen abspielen – allerdings ohne den 3D-Effekt."
---

- **Spatial Video, VR180, 3D-Foto und volumetrisches Video unterscheiden sich vor allem in räumlicher Tiefe, Blickfreiheit und technischer Komplexität.** **Während 3D-Fotos statische Tiefe abbilden, erweitern VR180 und Spatial Video dies um Bewegung, und volumetrisches Video erlaubt echte freie Perspektiven im Raum.**
- **Mit zunehmender räumlicher Freiheit steigen Produktionsaufwand, Datenmenge und technische Anforderungen deutlich.** **Insbesondere volumetrisches Video erfordert spezielle Aufnahme-Setups, hohe Rechenleistung und komplexe Postproduktion, während 3D-Fotos und VR180 vergleichsweise effizient umsetzbar sind.**
- **Nicht jedes räumliche Format ist für jeden Anwendungsfall sinnvoll.** **Der Einsatz hängt davon ab, ob es um räumliches Verständnis, emotionale Nähe, Bewegung im Raum oder interaktive Betrachtung geht – und welche technischen Rahmenbedingungen zur Verfügung stehen.**

&nbsp;

**Ihr kennt die Situation: Ein Kind stellt Dir eine vermeintlich simple Frage. Du lachst, denkst nach das die "einfache Antwort" eine Menge mehr Hintergrudinfos umfasst. **

**So ging es mir bei der Zuschauerfrage "Was ist eigentlich der Unterschied zwischen VR180 und Spatial Video?" So kam ich wieder vor die Kamera und habe in einem Video gleich ein paar weitere Begriffe erklärt.**

Daher... greift wieder zu einer Tasse Tee oder Kaffee und lest Euch die Erklärungen durch *oder schaut Euch das Video an*. Viel Spaß!

| Format | Räumliche Tiefe | Blickfreiheit | Aufwand | Typischer Einsatz |
| --- | --- | --- | --- | --- |
| 3D-Foto | Stereo (fest) | Kein (Standbild) | Niedrig | Produkt, Portrait, Social |
| VR180 | Stereo (180°) | Begrenzt (180°) | Mittel | Events, Interviews, B2B |
| 360°-Video | Keine echte Tiefe | Vollständig (360°) | Mittel | Rundgänge, Reportage |
| Spatial Video | Echte Stereo-Tiefe | Kamerarichtung | Mittel | Apple Vision Pro, Memories |
| Volumetrisches Video | Volle 3D-Tiefe | Frei (360° + Tiefe) | Sehr hoch | Hologramm, Industrie |

## Spatial Video – Einführung & Definition

**Spatial Video ist ein stereoskopisches Videoformat von Apple, das auf dem iPhone 15 Pro aufgenommen und auf Apple Vision Pro in echter Raumtiefe wiedergegeben wird.**

Das Format "Spatial Video" ist eine Weiterentwicklung bestehender Formate, mit denen sich "räumliche Videos" generieren lassen. Deswegen darf ich ein bisschen ausholen, um verständlich zu machen, an welchem Punkt in der Generierung von 3D-Bildern/-Videos das Format Spatial Video einsetzt.

*Statt langen Text lesen, hier alle Infos als Video...*

[Embed: https://www.youtube.com/watch?v=VyHZuwsBkss](https://www.youtube.com/watch?v=VyHZuwsBkss)

## **3D-Video & Stereoskopie: Grundlagen des räumlichen Videos**

**3D-Video nutzt zwei minimal versetzte Bildperspektiven (Stereoskopie), um dem Gehirn räumliche Tiefe zu signalisieren.**

Spatial Video ist ein Format, das zur Familie der 3D-Videos (Stereoskopie) gehört. Die theoretische Grundlage kommt aus dem menschlichen Kopf. Um dreidimensional sehen zu können, braucht es mindestens zwei nahestehende Blickwinkel in die gleiche Richtung = Stereoskopie. Das Gehirn packt die visuellen Daten übereinander und synchronisiert sie zu einer räumlichen Wahrnehmung. 

![Artikelinhalte](/assets/images/2025/11/Gitternetze.png)
*Die Gitternetze zeigen die Positionen von zwei "Augen" = Kameras für ein 3D-Foto*

3D-Videos nehmen dem Gehirn diese Arbeit ab. Eine Software verarbeitet beide Perspektiven und produziert daraus ein dreidimensionales Video. Das können auch Menschen mit nur einem Auge wahrnehmen.

## 3D-Foto: räumliche Fotos erklärt

**Ein 3D-Foto besteht aus zwei leicht versetzten Standbildern und wirkt auf kompatiblen Displays sowie in sozialen Netzwerken räumlich.**

Das gleiche Prinzip funktioniert natürlich auch bei 3D-Fotos. Funfact: Seit etwa 2017 wird jedes Foto, das mit einem iPhone gemacht wird, als 3D-Foto gespeichert. 

[Embed: https://www.youtube.com/watch?v=Ferb72l1MrY](https://www.youtube.com/watch?v=Ferb72l1MrY)

Bitte jetzt nicht nach einer entsprechenden Apple Pressemitteilung suchen. Eine offizielle Verlautbarung hat es dazu glaube ich nie gegeben. Facebook hat das genutzt und eine Funktion entwickelt, die diese Datenanordnung sichtbar macht. Kennen wir alle. 

## VR180 & 360°-Video: Unterschiede und Nutzung

**VR180 deckt 180 Grad mit echter Stereo-Tiefe ab, während 360-Grad-Video zwar rundum filmt, aber keine echte Tiefenwahrnehmung erzeugt.**

Beim VR180 Format nimmt ein spezielles Objektiv der Kamera in jede Richtung (oben, unten, links und rechts) mehr auf, als normalerweise bewusst optisch wahrgenommen wird. Ihr kennt das natürlich schon. Im entsprechenden Bild oder Video kann z. B. mit der Mouse das Blickfeld um bis zu 180 Grad geschwenkt werden. Ein Klassiker ist mittlerweile das 360 Grad Video. Hier kann man sein Sichtfeld um 360 Grad drehen und kippen.

## Spatial Video vs. klassische stereoskopische Formate

**Im Vergleich zu klassischem Kino-3D ist Spatial Video direkt für Apple-Hardware optimiert und liefert eine weichere, alltagstaugliche Tiefenwirkung.**

Beim stereoskopischen Format ist der gesamte Bildbereich vollständig dreidimensional aufbereitet. Die eigene virtuelle Drehbewegung des Betrachters ist dabei auf einen virtuellen Standpunkt festgelegt. Oder anders gesagt: Alles dreht sich um mich. 

Beim Spatial Format nimmt die Kamera mehr auf, als auf einen Blick sichtbar ist. Das heißt: Nicht alles, was aufgenommen wurde, ist vollständig sichtbar. Man blickt auf das Bild/Video wie durch ein Fenster. Der 3D-Effekt wird durch Tiefenunschärfe verstärkt. Der Fokus liegt auf einem Ausschnitt in der Mitte des Bildes. Wenn man den Bildschirm leicht nach links oder rechts dreht, rücken neue Bildausschnitte ins Blickfeld, die vorher außerhalb waren. 

![Artikelinhalte](/assets/images/2025/11/Fensterszene.png)
*Die Fensterszene aus dem Video*

Man kann hier gewissermaßen sich nach vorne aus dem »Fenster« beugen und um die Ecke schauen. Im Prinzip werden hier VR180 und das 360 Grad Video miteinander kombiniert, um einen noch stärkeren dreidimensionalen Eindruck zu erhalten. 

## **Meta Quest & Spatial Video = technische Missverständnisse**

**Meta Quest und Apple Vision Pro unterscheiden sich in Auflösung, Tracking-Ansatz und Einsatzschwerpunkt grundlegend.**

Übrigens hat Meta diesen Unterschied nicht verstanden. Ein Video im Spatial Format, das auf Meta Quest 3 läuft, ermöglicht genau dieses Sichtbarmachen »unsichtbarer« Randbereiche nicht. Es ist also nichts anderes als ein stereoskopisches Video im quadratischen Format. 😉

## Technik: Wie Spatial Video als Dateiformat funktioniert

**Spatial Video wird als MOV-Datei mit zwei H.265-Streams und eingebetteten Tiefen-Metadaten gespeichert.**

Bei einem stereoskopischen Format liegen zwei Bilder vor – wie beim linken und rechten Auge. Diese beiden Bilder werden synchronisiert. Allerdings bedeutet das, dass die beiden Ausgangsbilder je nachdem, ob sie vertikal oder horizontal zueinander angeordnet sind, auf Pixel in der Breite oder Höhe verzichten, um in der Addition ein normales, nicht verzerrtes Ausgabeformat beizubehalten. 

Apple wollte aber nicht auf entsprechend limitiertes Ausgangsbildmaterial zugreifen und hat deswegen ein anderes Prinzip eingeführt, um ein dreidimensionales Bild zu generieren.

Für das Spatial Format wird nur eines der beiden Ausgangsbilder genutzt. Dann sucht die Software im Vergleich mit dem zweiten Bild nach Unterschieden zwischen beiden, um hieraus nur die Informationen zu verwenden, die für die räumliche Perspektive wichtig sind.

Zur Verdeutlichung: Ihr blickt auf einen Stuhl vor einer Wand. Die Wand ist zweidimensional. Jedes einzelne Pixel in der Wand sieht durch beide »Augen« gleich aus. Anders verhält es sich mit dem davor stehenden Stuhl. Es ist ein räumlicher Gegenstand und je nach Blickwinkel stehen Flächen, Kanten und Erhebungen in einem anderen Verhältnis zueinander. Der Stuhl sieht in beiden »Augen« unterschiedlich aus. 

In der Wahrnehmung führt das dazu, dass Hintergründe wie eine Wand oder auch weiter weg stehende Gegenstände/Personen etwas unscharf erscheinen. Diese Unschärfe kaschiert die fehlende Dreidimensionalität im Bildhintergrund. Mehr zu dem ganzen Thema von [Mike Swanson](https://www.linkedin.com/in/mikeswanson/) [Spatial Video](https://blog.mikeswanson.com/post/744337392062873601/spatial-video).

## Spatial Video auf Apple Vision Pro – erste Perspektiven

**Auf Apple Vision Pro werden eigene Spatial Videos automatisch als immersives 3D-Erlebnis im passenden Wiedergabemodus abgespielt.**

Ich gehe fest davon aus, dass wir uns demnächst über die Apple Vision Pro alle unsere gespeicherten Fotos und Videos in 3D anschauen können. Also auch ältere. Denn die dafür notwendige Art der Datenspeicherung existiert ja bereits. 

- **Typisch Apple 1:** Erst mal die Voraussetzungen in Form von Datenverarbeitung und Datenspeicherung abschließen – und dann an die Entwicklung von Visionen gehen. 
- **Typisch Apple 2:** Abwärtskompatibilität bei den Datenträgern. Neue Software soll eine relativ niedrige technische Einstiegshürde haben, damit möglichst viele Menschen sie direkt mit vorhandenen Devices nutzen können. 
- **Typisch Gerd:** Eine entsprechende Funktion für iPhones und iPads wird bald kommen. Das passt zu Apple, das macht Sinn. Natürlich halte ich Euch dahingehend auf dem Laufenden.

## Apple Vision Pro – Auflösung und räumliche Wirkung

**Apple Vision Pro zeigt Spatial Video mit bis zu 4K pro Auge bei 30 fps — ausreichend für überzeugende räumliche Wahrnehmung.**

Bisher im Handel erhältliche Wiedergabegeräte erlauben eine maximale Auflösung von bis zu 8 k. Die Apple Vision Pro wird bis zu 14 k möglich machen. Das in Verbindung mit dem Spatial Video-Format wird sich in der optischen Wirkung potenzieren. *Funfact: Gerade hat ein mit Apple zusammenarbeitender Kamerahersteller eine 16K Kamera angekündigt.*

## Volumetrisches Video – nächste Stufe der räumlichen Darstellung

**Volumetrisches Video erfasst ein Objekt aus allen Richtungen und erzeugt ein frei begehbares 3D-Modell statt einer fixen Kameraperspektive.**

Nein, auch das Spatial Format ist nicht das Ende der Fahnenstange. Die nächste Stufe ist das volumetrisches Bildformat. 

Hier wird ein Ereignis von einem im 360 Grad Radius aufgebauten Multi-Kamerasystem rundherum aufgenommen. Das bedeutet im Ergebnis, ich werde auf einem Bildschirm oder mit einer VR-Brille in das Geschehen hineingehen und dort jeden beliebigen Blickwinkel einnehmen können. In der AR-Variante kann ich beispielsweise ein Fußballspiel z. B. auf meinem Tisch stattfinden lassen – wie ein Hologramm. Dafür gibt es bereits Anschauungsmaterial, das Ihr Euch im obigen Video anschauen könnt. 

&nbsp;

&nbsp;

Die Unterschiede zwischen Spatial Video, VR180, 3D-Foto und volumetrischem Video lassen sich technisch klar beschreiben. In der Praxis stellt sich jedoch oft die nächste Frage: Wie werden solche Formate sauber produziert, kombiniert und in bestehende Systeme eingebunden?

Auf einer separaten Unterseite beschreiben wir, wie wir [VR180- und 360°-Videos technisch umsetzen](https://visales.de/vr180-360videos/), welche Aufnahmeszenarien sich dafür eignen und wie diese Formate weiterverwendet werden können – etwa für Präsentationen, Web-Einbindungen oder VR-Anwendungen.

Die hier beschriebenen Formate werden häufig kombiniert eingesetzt. Insbesondere VR180- und 360°-Videos dienen dabei als praktikabler Einstieg in räumliche Videoformate.

Gern auch neue Fragen, die ich so wie diese Fragen aufgreifen kann. ❤️

Viele Grüße aus Velbert,

Gerhard Schröder 

&nbsp;

&nbsp;

&nbsp;

&nbsp;

### **Du willst wissen ob AR & 3D für dein Produkt passt?**

In 30 Minuten sortieren wir gemeinsam, ob und wo AR in eurem Vertrieb konkret etwas bringt — ohne Pitch, ohne Angebot. Kein Verkaufsdruck, eine ehrliche Einordnung. **Rheingas und Somfy haben so angefangen.**

[Einordnungsgespräch buchen](https://visales.de/kontakt/){.gh-button .cta-center}

&nbsp;

&nbsp;

## Typische Entscheiderfragen

→ Praxiseinsatz AR im Vertrieb: [AR im B2B-Vertrieb bei viSales](https://visales.de/augmented-reality/)


